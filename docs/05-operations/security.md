...
| **No passkeys yet.** ADR-0007 makes WebAuthn the *preferred* factor; only the TOTP fallback exists. `user_passkeys` is created and classified but unwritten | Owners on a modern phone are asked for six digits when a fingerprint would do, which is the enrolment-rate risk ADR-0007 names as its own falsification condition | Passkey slice |
| **No 2FA disenrolment or reset path.** Re-enrolling over a live factor is refused (a stolen session must not swap the factor out), and FR-014b's support-channel reset with its 48h delay is deferred | A farmer who loses their phone **and** all ten recovery codes cannot be recovered by us at all | FR-014b, later phase |
