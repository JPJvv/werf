/**
 * The SINGLE SOURCE for tenancy in sync. PowerSync sync rules are a SEPARATE system from
 * Postgres RLS with a silent failure mode: a permissive sync rule leaks farm B's rows onto
 * farm A's phone even when every RLS policy is perfect, because replication bypasses the
 * query path RLS protects. So we derive the sync rules and the RLS predicates from ONE table
 * here (`TENANCY`); they cannot disagree by construction, and test/tenancy.spec.ts proves the
 * cross-farm leak is closed. See .claude/rules/db.md.
 *
 * `local-schema.ts` also derives the client's local SQLite schema from this registry — a
 * table classified `server-only` here gets no local table, and a `neverSyncColumns` entry
 * never becomes a local column. One registry, three consumers (sync rules, RLS, local schema)
 * that cannot drift apart.
 *
 * Phase 1 classifies the identity & tenancy core; Phase 2 adds livestock.
 */
