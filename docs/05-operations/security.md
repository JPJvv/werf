...
| **A stolen session can ADD a passkey.** TOTP re-enrolment is refused outright for exactly this reason, but `POST /auth/2fa/passkey` has no step-up check and is reachable pre-enrolment | An attacker who steals a live session can plant a second, persistent credential. The owner can see it in the device list and revoke it (FR-014c), but nothing alerts them | Step-up re-authentication before enrolling a factor |
