...
3. **Provenance + a verified flag travel with the figure.** A `regulatory_rates` row carries its
   ...
   Phase-3-opening slice — see STATUS §5. Until it exists, "do not deploy" is enforced by review,
   not by code.)
...
5. **A `compliance-checker` pass is about RULES, not figures.** `pnpm verify` cannot tell you
   ...
   payroll slice (even with placeholder figures), then batched per-phase. See STATUS §6.
...
|---|---|---|---|---|---|---|
| `NMW_FARM` national minimum wage | R30.23 / hour (eff. 1 Mar 2026) | CITED | Gov. Gazette No. 54075, 3 Feb 2026 | — | **2027-02** (watch Feb gazette, deploy before 1 Mar) | Phase 3 payroll |
| `BCEA_THRESHOLD` earnings threshold | R269,600.90 / yr (R22,466.74 / mo, eff. 1 May 2026) | CITED | GN 7384, Gazette 54544, 17 Apr 2026 | — | **2027-04** (watch Apr gazette, deploy before 1 May) | Phase 3 payroll (gates overtime/Sunday entitlement) |
| `UIF_CEILING` monthly UIF ceiling | not yet captured | PLACEHOLDER | UIF Act (ceiling gazetted separately) | — | **capture before Phase 3 payroll** | Phase 3 payroll (1% ee + 1% er, capped) |
| Overtime multiplier | 1.5× (max 10 h/week) | CITED | BCEA s10 | — | on amendment (rare) | Phase 3 payroll |
| Sunday work | 2× (1.5× if ordinarily Sundays) | CITED | BCEA s16 | — | on amendment (rare) | Phase 3 payroll |
| Public holiday | 2× if not ordinarily worked | CITED | BCEA s18 | — | on amendment (rare) | Phase 3 payroll |
| Deduction cap: accommodation | max 10% of wage | CITED | Sectoral Determination 13 | — | occasionally | Phase 3 payroll (warn on exceed) |
| Deduction cap: food | max 10% of wage | CITED | Sectoral Determination 13 | — | occasionally | Phase 3 payroll (warn on exceed) |
| Leave entitlements (annual / sick / maternity / family) | per §2.2 | CITED | BCEA s20/s22/s25/s27 | — | on amendment (rare) | Phase 3+ leave module |
| Public holidays list (per year, incl. proclaimed once-off) | not yet captured | PLACEHOLDER | proclamations | — | **each year, ongoing** | Phase 3 payroll |
...
- [ ] **Before Phase 3 payroll opens** — capture `UIF_CEILING` and the current-year public-holiday list; run one `compliance-checker` pass on the first payroll slice.
...
| Date | Change |
|---|---|
| 2026-07-30 | Register created (JP's compliance operating model). Seeded from `legal-compliance.md` §2.2; all wage/threshold figures marked `CITED` (re-verify before Phase 3 deploy). No figure independently re-verified this cycle. |
